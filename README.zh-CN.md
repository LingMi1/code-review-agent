...
> ⚠️ 以下数据来自较早的 15 个用例运行，**已过时**——未覆盖当前的 30 个用例。重跑 `go run ./cmd/eval/ -real` 即可刷新。

| 指标 | Mock 基准（30 用例） | DeepSeek（15 用例运行，已过时） |
|---|---|---|
| 通过率（F1 ≥ 0.5） | 73%（22/30） | **73%（11/15）** |
| Macro Precision | 0.69† | **0.51** |
| Macro Recall | 0.74† | **1.00*** |
| Macro F1 | 0.71† | **0.68** |

> † Mock 基准的 Macro 指标为 30 用例下的估算值（新增 6 负例 + 6 多文件用例均按 F1=1 计入），运行 `go run ./cmd/eval/` 可得到精确值。
>
